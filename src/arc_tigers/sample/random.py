...
class RandomSampler:
    def __init__(self, eval_data: Dataset, seed: int | None = None, **kwargs):
        ...
        self._sample_order = rng.permutation(np.arange(len(eval_data)))
        self._sample_prob = np.arange(1, len(eval_data) + 1) / len(eval_data)
        self.n_sampled = 0
    ...
    @property
    def sample_prob(self) -> npt.NDArray[np.float64]:
        """
        Returns:
    ...
    def sample(self) -> tuple[np.integer, np.floating]:
        """

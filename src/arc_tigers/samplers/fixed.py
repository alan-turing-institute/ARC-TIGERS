...
class FixedReplaySampler(Sampler):
    """
    Wrapper sampler that replays a sampling sequence from a FixedSampler.

    This sampler is used internally by the replay system to integrate with
    the existing sampling framework while using predetermined sequences.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initialize the replay sampler.

        Args:
            eval_data: The evaluation dataset
            seed: Random seed (for compatibility, not used)
            fixed_sampler: The FixedSampler instance to replay
            **kwargs: Additional arguments (ignored)
        """
        super().__init__(eval_data, seed, "fixed_replay")
        # Copy the state from the pre-created fixed sampler
        self.n_sampled = fixed_sampler.n_sampled
        self._sample_order = fixed_sampler._sample_order.copy()
        self._sample_prob = fixed_sampler._sample_prob.copy()

    @property
    def labelled_idx(self) -> list[int]:
        """Return indices of samples that have been labelled."""
        return self._sample_order[: self.n_sampled]

    @property
    def unlabelled_idx(self) -> list[int]:
        """Return indices of samples that haven't been labelled."""
        return self._sample_order[self.n_sampled :]

    @property
    def sample_prob(self) -> list[float]:
        """Return probabilities of labelled samples."""
        return self._sample_prob[: self.n_sampled]
    ...
    def sample(self) -> tuple[int, float]:
        ...
        if self.n_sampled > len(self._sample_order):
            ...
            raise IndexError(msg)
        return (
            self._sample_order[self.n_sampled - 1],
            self._sample_prob[self.n_sampled - 1],
        )
